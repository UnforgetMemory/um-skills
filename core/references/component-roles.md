...
> 本文档定义每个组件的职责边界。分层总览（七层模型）与链路示例见 [architecture-principles.md](architecture-principles.md) 和 [modern-architecture.md](modern-architecture.md)。
> 注：组件 ≠ 层。层是 app/feature/store/scenario/provider/engine/libraries 七层；组件是层内的职责模式（如 Pipeline 位于 scenario 层内）。
...
| **Pipeline** | 多阶段数据怎么处理 | 数据流水线编排 | 不包含 UI/业务状态 |
...
**示例**：`SubtitlePipeline: Extract → Recognize → Translate → Render`
...
Pipeline = 数据流水线（Data Pipeline）— scenario 层内的组件模式
