...
## 硬性纪律
1. 禁止自动 commit/push：每次执行前必须展示 PLAN + 完整消息；Q4=仅本次 时须经面板 Q2 确认，push 仅面板 Q3 显式选择（条件执行，失败暂停回报）；未经确认 → BLOCKED
2. 效力语义：仅本次确认单次有效（下一轮/内容变更须重新确认）；Q4=本会话允许 时同动作免逐次面板（展示不豁免），授权可随时撤销（人工确认协议见 `references/prohibitions.md`）
3. 安全审计 Critical/High → Hard Block，禁止提交
4. 只提交本次变更范围，禁止修改无关模块
5. 版本源以官方文件优先，记忆索引只是硬编码同步清单
...
| P5 | 提交消息生成 + 面板确认 | references/decision-panel.md | 交互工具 | 面板 Q2（当轮 P7 硬前置） |
| P6 | 暂存+审计（add 后 commit 前） | references/security-audit.md | grep/命令 | Hard Block |
| P7 | 执行提交：--signoff -F 消息文件；逐组验证 | — | 环境命令 | Q2 已确认（当轮）；log 验证 |
| P8 | 硬编码同步：按索引短路径逐处更新→回写索引 | references/project-memory.md | 文件工具 | 索引核对 |
| P9 | 推送：面板 Q3 意愿→条件执行→验证 | — | 环境命令 | Q3 显式选择（当轮） |
...
Q2 提交确认：[确认 N 个 commit, 修改意见]（已展示 PLAN+消息；修改意见 → 修订后重新面板确认）
Q3 推送意愿：[提交后推送, 仅本地提交]（条件执行，commit 失败则暂停回报）
Q4 确认效力：[仅本次, 本会话允许]（默认仅本次；本会话允许 = commit/push 免逐次确认，内容展示不豁免；会话结束失效）
```
...
4. P8：硬编码索引核对完成（无遗漏）
5. P9：仅 Q3 显式选择后 push；终态 git status 干净
6. Q2/Q3/Q4 确认记录（面板编号 + 用户选项 + 效力层级）已写入报告
